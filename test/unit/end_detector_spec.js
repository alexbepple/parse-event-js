...
describe('End detector', function() {
	it('returns the result of the date detector for the input minus first token', function() {
		// rewrite test so that error messages are understandable!!!
		// also this test should not fail when tail processing is wrong
		var stubDateDetectorResult = {
			date: Date.create(),
			tail: 'a tail'
		};
		var stubDetectDate = function(input) {
			if (input === 'bar') return _.clone(stubDateDetectorResult);
			return {date: Date.create('invalid'), tail: 'invalid result'};
		};
		var detectEnd = endDetector.detector(stubDetectDate);

		detectEnd('foo bar').should.eql(stubDateDetectorResult);
	});
	it('uses tail of date detector when it finds a date', function() {
		...
		detectEnd('input').tail.should.eql('tail from date detector');
	});
